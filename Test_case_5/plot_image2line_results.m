function Theta_test =plot_image2line_results(Xi,z_input_all_test,f_output,f_output_test,Theta,N2,N,x,y,x_train1,y_train1,index_input_test,index_input_train,y_predict,y_predict_ood,N2_train,error_file,  soln_file,x_wss,N_wss)


soln_sindy_train = Theta * Xi;


[Theta_test, eqn_list] = poolData_image2line(N2,N,z_input_all_test,x,y,x_train1,y_train1,x_wss,N_wss);   
...
if (1)
   timestep = 300; %or any desired


   figure;
   soln_reshaped = reshape(f_output,[N_wss,N2_train]);
   soln = soln_reshaped(:,timestep);
   plot(x_wss, soln,'DisplayName','True solution');  %training data plot
   hold on;

   soln_reshaped = reshape(soln_sindy_train,[N_wss,N2_train]);
   soln = soln_reshaped(:,timestep);
   plot(x_wss,soln,'k--', 'DisplayName', 'Interpretable model solution');  %training data plot
   hold on;

   soln_reshaped = reshape(y_predict,[N_wss,N2_train]);
   soln = soln_reshaped(:,timestep);
   plot(x_wss,soln,'r--','DisplayName', 'Neural network solution');  %training data plot
   ylabel('WSS');
   xlabel('x');
   hold off;
   legend
