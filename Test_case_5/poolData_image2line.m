function [yout,Equation] = poolData_image2line(N2,N,z_input_all,x,y,x_train1,y_train1,x_wss,N_wss)
%image 2 image
...
yout = zeros(N2*N_wss,N_t ); % (number of files * total number of pt,  number of integral terms)
x_temp = zeros(NN,NN);
...
for kk=1:N2
  ...
  %%%%%%%%%%%%
   for j=1:N_e   %RBF Kernel
         mygreen = exp(- ( (x_train1(:,:) - x_wss(ii)).^2 +  (y_train1(:,:) - 0).^2   ) / exp_factors(j) );   %------ 2nd green function in the library %%%%%%
         %x_temp(:,:) = z_input_all(kk,:,:);
   ...
   for j=1:N_e   %http://www.greensfunction.unl.edu/glibcontent/node24.html
         mygreen = exp(- sqrt( (x_train1(:,:) - x_wss(ii)).^2 +  (y_train1(:,:) - 0).^2   ) / exp_factors(j) );   %------ 2nd green function in the library %%%%%%
         %x_temp(:,:) = z_input_all(kk,:,:);
         int_fn =  mygreen .* x_temp; 
         yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
         if(i==1) %save the equation
  ...
  Dist_thresh = 1; % default: 1
  for j=1:N_e
           Dist = sqrt( (x_train1(:,:) - x_wss(ii)).^2 +  (y_train1(:,:) - 0).^2   )/ (exp_factors(j)/2);
           mygreen = ones(NN,NN) ; 
           mygreen(Dist>Dist_thresh) = 0;
           int_fn =  mygreen .* x_temp; 
           yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
           if(i==1) %save the equation
...
%          %Polynomial order 1
%          mygreen1 = x_train1(:,:) * x_wss(ii) + y_train1(:,:) * 0   ;   %------ 2nd green function in the library %%%%%%
%          %x_temp(:,:) = z_input_all(kk,:,:);
...
%           n_green_funcs = n_green_funcs + 1;
                     ...
                     %%%%%%%%%%%%%%% 
         for j=1:N_e
         mygreen =  exp(- (x_train1(:,:) - x_wss(ii) )  /exp_factors(j));   %------ 22 green function in the library %%%%%%
         %x_temp(:,:) = z_input_all(kk,:,:);
         int_fn =  mygreen .* x_temp; 
         yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
          if(i==1) %save the equation
         ...
         n_green_funcs = n_green_funcs + 1;
                ...
                %%%%%%%%%%%%%%% 
         for j=1:N_e
         mygreen =  exp(- (y_train1(:,:) - 0 )  /exp_factors(j));  %------ 23 green function in the library %%%%%%
         %x_temp(:,:) = z_input_all(kk,:,:);
         int_fn =  mygreen .* x_temp; 
         yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
           if(i==1) %save the equation
...
%          for j=1:N_e
%          mygreen =  exp(- (x_train1(:,:) - x_wss(ii) ).^2  /exp_factors(j));   %------ 22 green function in the library %%%%%%
%          %x_temp(:,:) = z_input_all(kk,:,:);
%          int_fn =  mygreen .* x_temp; 
%          yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
%          n_green_funcs = n_green_funcs + 1;
...
%                 %%%%%%%%%%%%%%% 
%          for j=1:N_e
%          mygreen =  exp(- (y_train1(:,:) - 0 ).^2  /exp_factors(j));  %------ 23 green function in the library %%%%%%
%          %x_temp(:,:) = z_input_all(kk,:,:);
%          int_fn =  mygreen .* x_temp; 
%          yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
%          n_green_funcs = n_green_funcs + 1;
...
%           mygreen = ( x_train1(:,:) - x_wss(ii) ).^2 ;
%           int_fn = mygreen .* x_temp ; 
%           yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
%           n_green_funcs = n_green_funcs + 1;
% 
%           mygreen = ( y_train1(:,:) - 0).^2 ;
%           int_fn = mygreen .* x_temp ; 
%           yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
%           n_green_funcs = n_green_funcs + 1;
...
%          mygreen = (1 + sqrt(6/exp_factors(j)*sqrt( (x_train1(:,:) - x_wss(ii)).^2 +  (y_train1(:,:) - 0).^2   )   ) ) .* exp(- sqrt(6/exp_factors(j)*sqrt( (x_train1(:,:) - x_wss(ii)).^2 +  (y_train1(:,:) - 0).^2   )  ));   %------ 2nd green function in the library %%%%%%
%          %x_temp(:,:) = z_input_all(kk,:,:);
%          int_fn =  mygreen .* x_temp; 
%          yout(i,n_green_funcs) = trapz(y,trapz(x, int_fn'  ,1),2);
%           n_green_funcs = n_green_funcs + 1;
